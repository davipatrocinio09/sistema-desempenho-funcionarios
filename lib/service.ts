...
export async function member(): Promise<Member | null> {
  ...
  const db = database();
  return await db.prepare('SELECT u.id,u.email,u.name,u.role,u.active FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.id=? AND s.expires_at>? AND u.active=1').bind(await sessionTokenHash(token),new Date().toISOString()).first<Member>();
}
